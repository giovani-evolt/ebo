...
class SettlementExtension implements QueryCollectionExtensionInterface, QueryItemExtensionInterface
{
    ...
    private function addSellerFilter(QueryBuilder $qb, string $resourceClass): void
    {
        ...
        if (!property_exists($resourceClass, 'settlement')) {
            return;
        }
        ...
        $qb
            ->join(sprintf('%s.settlement', $alias), 'st')
            ->andWhere(sprintf('st.seller = :seller', $alias))
